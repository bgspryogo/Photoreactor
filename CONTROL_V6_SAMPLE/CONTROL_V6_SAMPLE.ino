...
void controlOutputs() {
  ...
  if (inlet_toggle == false || WATER_LEVEL == 2) {
    ...
    if (WATER_LEVEL == 2 && inlet_toggle == true && !inletRecentlyPressed) {
      ...
      if (WATER_LEVEL != 2) {
        inletAutoStopped = false;  // Reset when water level changes
      }
    }

    Serial.println("FILLING MAIN TANK FINISHED");
  } 
  else if (inlet_toggle == true) {
    digitalWrite(INPUT_SOLENOID, LOW);
    digitalWrite(INLET_PUMP, LOW);
    inletCommandSent = false;
    // Don't reset inletAutoStopped here - let water level change do it
...
if (outlet_toggle == false || WATER_LEVEL == 1) {
  ...
  if (millis() - outletDelayStart >= 100000UL) {
